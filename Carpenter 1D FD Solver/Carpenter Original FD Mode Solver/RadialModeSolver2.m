...
c = 299792458;
%n_r = fiber.nr;
% mesh radial coordinates
% if (isConstantStep==0)
%     r = GenRadialDiffEqMesh(fiber,dr_max,lambda,L);
% else
%     r = 2.0.*(0:0.0001:1).*fiber.a;
% end
%index = feval(fiber.function,r,fiber);
...
% if (nargout > 2)
%     
%     % compute group-delay (i.e. d beta / d omega) via pertubation method
%     
%     DeltaF = 1/2 * (1 - (index/fiber.n1).^2);
%     P = (2 - DeltaF*(fiber.y + 4));
%     
%     for j = 1:length(beta)
%         [RH_vec,dmp] = eigs(A',1,beta(j)^2,options);
%         R_vec = R(:,j);
%         [dmp,idx] = max(abs(RH_vec),[],1);
%         RH_vec = RH_vec ./ RH_vec(idx); % RH vec is vector,
%         num = RH_vec'.* P * R_vec;
%         den = RH_vec' * R_vec;
%         tau(j) = fiber.length * fiber.n1 / (2*c) * ...
%             (fiber.n1 * k0 / beta(j)) * num/den;
%     end
% end

%tau=[]; % turning off the calculation for propagation time of each mode
...
% put radial position as first column of output matrix R
%R(:,2:end+1) = R;
%R(:,1) = r';